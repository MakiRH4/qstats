...
const fetchAndBroadcast = async (): Promise<void> => {
  for (const { res, wallet_id, data } of clients) {
    let quTrans: QuTransaction;
    try {
        ...
        quTrans = await response.json();
        let input: number = 0; 
        let output: number = 0; 
        if (quTrans && quTrans.events ){
          quTrans.events.forEach(element => {
            const difftime = (quTrans.latestTick - element.tick)*30;
            if (element.sourceId == wallet_id)
              input += Number(element.amount);
            if (element.destinationId == wallet_id)
              output += Number(element.amount);
            element.createdAt = new Date((nowInSeconds - difftime) * 1000);
          });
          quTrans.walletId = wallet_id;
          quTrans.total = output -  input;
      } else 
      {
        ...
        broadcastEvent(res, 'new_data', quTrans);
    } catch (error) {
...
export default function handler(req: NextApiRequest, res: NextApiResponse): void {
  if (req.method === 'GET') {
    ...
    clients.push({ res, wallet_id , data: null});
